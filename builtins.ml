...
let eval_add arg1 arg2 = match arg1, arg2 with
  | Atom s1, Atom s2 -> Atom (string_of_int (int_of_string s1 + int_of_string s2))
  | _ -> raise (Error "Les arguments doivent être de type Atom")
...
let eval_inf arg1 arg2 = match arg1, arg2 with
  | Atom s1, Atom s2 -> if int_of_string s1 <= int_of_string s2 then Atom "t" else Atom "nil"
  | _ -> raise (Error "Les arguments doivent être de type Atom")

let eval_sup arg1 arg2 = match arg1, arg2 with
  | Atom s1, Atom s2 -> if int_of_string s1 >= int_of_string s2 then Atom "t" else Atom "nil"
  | _ -> raise (Error "Les arguments doivent être de type Atom")

let eval_add2 arg =
  let rec aux a =  match arg with
    | Atom s1 -> int_of_string s1
    | Cons (Atom s1,cdr) -> (aux car) + (aux cdr)
...
let all_builtins =
  [ (* Special forms *)
    ...
    "inf", BuiltinFn2 eval_inf;
    "sup", BuiltinFn2 eval_sup;
    "add2", BuiltinFn1 eval_add2
]
